...
| --- | --- | --- |
| Committed source, architecture and automated conformance | `0.7.0` [release qualification](results-release-2026-09-08.md), with exact commit and workflow links | Keep the released source and reviewed architecture baseline aligned; qualify any changed inputs |
...
| New-process and HTTP delivery | Real children, loopback HTTP/TLS, CLI/MCP, withheld-output and cancellation tests with synthetic consent | [Native registration and per-use deny/allow](process.md), recipient-side completion and full native dialog readability |
