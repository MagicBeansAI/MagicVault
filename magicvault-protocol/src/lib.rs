...
impl Drop for Envelope {
    fn drop(&mut self) {
        if let Some(token) = &mut self.token { token.zeroize(); }
    }
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct PairRequest { pub label: String }

#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct EnrollRequest { pub label: String, pub field_names: Vec<String> }

#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct AccessRequest { pub credential_ref: String }

#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ApprovalQuery { pub approval_id: Uuid }

#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct RevokeRequest { pub client_id: Uuid }

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
pub enum ErrorCode {
    InvalidRequest, UnsupportedVersion, Unauthorized, StaleSession, Busy,
    Denied, Unavailable, NotFound, Conflict, Capacity, Expired,
    PersistenceUncertain, TransportUnavailable, TransportUncertain,
}

#[derive(Serialize, Deserialize)]
#[serde(tag = "status", content = "result", rename_all = "snake_case", deny_unknown_fields)]
pub enum Reply { Ok(Response), Error(ErrorCode) }
...
#[derive(Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct Pairing { pub client_id: Uuid, pub token: String }
...
#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
pub enum Decision { Pending, Allowed, Denied, Expired, Uncertain }
...
pub fn valid_label(value: &str) -> bool {
    !value.trim().is_empty() && value.len() <= 80
...
pub fn valid_reference(value: &str) -> bool {
    value.len() == 41 && value.strip_prefix("cred_").is_some_and(|id| {
...
impl Request {
    pub fn validate(&self) -> Result<(), ErrorCode> {
        let valid = match self {
            ...
            Self::Enroll(p) => valid_label(&p.label) && !p.field_names.is_empty()
                && p.field_names.len() <= MAX_FIELDS
                && p.field_names.iter().all(|f| valid_name(f))
                && p.field_names.iter().collect::<std::collections::HashSet<_>>().len() == p.field_names.len(),
            Self::RequestAccess(p) => valid_reference(&p.credential_ref),
            _ => true,
        };
        if valid { Ok(()) } else { Err(ErrorCode::InvalidRequest) }
    }
}
