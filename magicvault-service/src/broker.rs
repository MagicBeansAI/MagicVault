...
use magicvault_core::{store::{SecretAuditEvent, SecretStore, SecretPartitionStatus, SecretSourceKind}, InjectionTarget, policy::SecretPolicy};
...
use crate::{human::HumanInteraction, storage::{self, InstanceLock}};
...
#[serde(deny_unknown_fields)]
struct Registry { version: u32, peers: Vec<Peer> }

#[derive(Clone)]
struct Auth { id: Uuid, digest: [u8; 32] }
...
impl Job {
    fn projection(&self, id: Uuid) -> ApprovalStatus {
        ApprovalStatus { approval_id: id, decision: self.decision, operation: "metadata_connection".into() }
    }
}
...
impl State {
    ...
    fn reap(&mut self, now: Instant) {
        ...
        for job in self.jobs.values_mut() {
            if job.decision == Decision::Pending && job.expires <= now { job.decision = Decision::Expired; }
        }
    }
}
...
impl Drop for SecretInput {
    fn drop(&mut self) { for value in self.0.values_mut() { value.zeroize(); } }
}
...
impl Broker {
    pub fn open(lease: Arc<InstanceLock>, human: Arc<dyn HumanInteraction>) -> Result<Arc<Self>, ErrorCode> {
        storage::validate_vault(lease.root())?;
        let key = storage::load_key(&lease.instance)?;
        let store = SecretStore::new(Box::new(key), lease.root().join("vault")).map_err(|_| ErrorCode::Unavailable)?;
    ...
    pub fn with_components(lease: Arc<InstanceLock>, store: SecretStore, human: Arc<dyn HumanInteraction>) -> Result<Arc<Self>, ErrorCode> {
        if store.base_dir() != lease.root().join("vault") { return Err(ErrorCode::InvalidRequest); }
        storage::validate_vault(lease.root())?;
        if store.partition_status(SecretSourceKind::Provisioned) != SecretPartitionStatus::Loaded
            || !store.feature_status(SecretSourceKind::Provisioned).is_available() {
            return Err(ErrorCode::Unavailable);
        }
        let credentials = store.list_available();
        if credentials.len() > MAX_CREDENTIALS || credentials.iter().any(|entry| {
            !valid_reference(&entry.id) || !valid_label(&entry.label)
                || store.provisioned_metadata(&entry.id).is_none_or(|metadata| {
                    metadata.field_names.is_empty() || metadata.field_names.len() > MAX_FIELDS
        ...
        let registry = match path.symlink_metadata() {
            ...
            Err(error) if error.kind() == std::io::ErrorKind::NotFound && credentials.is_empty() => Registry { version: 1, peers: Vec::new() },
            Err(_) => return Err(ErrorCode::Unavailable),
        };
        if registry.version != 1 || registry.peers.len() > MAX_CLIENTS
            || registry.peers.iter().any(|p| !valid_label(&p.label) || p.references.len() > MAX_CREDENTIALS || p.references.iter().any(|r| !valid_reference(r) || !store.contains_provisioned(r)))
            || registry.peers.iter().map(|p| p.id).collect::<BTreeSet<_>>().len() != registry.peers.len()
            || registry.peers.iter().map(|p| &p.label).collect::<BTreeSet<_>>().len() != registry.peers.len() {
            return Err(ErrorCode::Unavailable);
        }
        Ok(Arc::new(Self {
            lease, store, state: Mutex::new(State { registry, jobs: HashMap::new(), faulted: false }),
            human, human_gate: Arc::new(Semaphore::new(1)), epoch: Uuid::new_v4(), shutdown: CancellationToken::new(),
        }))
    }

    pub fn socket(&self) -> std::path::PathBuf { self.lease.socket() }
    ...
    async fn transaction<T: Send + 'static>(self: &Arc<Self>, operation: impl FnOnce(&Self, &mut State) -> Result<T, ErrorCode> + Send + 'static) -> Result<T, ErrorCode> {
        ...
        }).await.map_err(|_| ErrorCode::Unavailable)?
    }

    fn ready(&self, state: &State) -> Result<(), ErrorCode> {
        if state.faulted { return Err(ErrorCode::PersistenceUncertain); }
        if self.shutdown.is_cancelled() { return Err(ErrorCode::Unavailable); }
        Ok(())
    }

    fn peer<'a>(&self, state: &'a State, auth: &Auth) -> Result<&'a Peer, ErrorCode> {
        self.ready(state)?;
        state.registry.peers.iter().find(|p| p.id == auth.id && equal_digest(&p.token_hash, &auth.digest)).ok_or(ErrorCode::Unauthorized)
    }

    fn save(&self, state: &mut State) -> Result<(), ErrorCode> {
        let bytes = serde_json::to_vec(&state.registry).map_err(|_| ErrorCode::Unavailable)?;
        if bytes.len() as u64 > storage::MAX_STATE_BYTES { state.faulted = true; return Err(ErrorCode::PersistenceUncertain); }
        if write_bytes_durably_with_mode_sync(&self.lease.root().join("clients.json"), &bytes, Some(0o600)).is_err() {
            state.faulted = true;
    ...
    fn metadata(&self, reference: &str) -> Option<CredentialMetadata> {
        self.store.provisioned_metadata(reference).filter(|metadata| {
            valid_reference(&metadata.id) && valid_label(&metadata.label)
                && metadata.field_names.len() <= MAX_FIELDS
                && metadata.field_names.iter().all(|name| valid_name(name))
        }).map(|metadata| CredentialMetadata {
            credential_ref: metadata.id, label: metadata.label, field_names: metadata.field_names,
        })
    }

    pub async fn execute(self: &Arc<Self>, envelope: Envelope) -> Reply {
        match self.dispatch(envelope).await { Ok(response) => Reply::Ok(response), Err(error) => Reply::Error(error) }
    }

    async fn dispatch(self: &Arc<Self>, envelope: Envelope) -> Result<Response, ErrorCode> {
        if envelope.version != VERSION { return Err(ErrorCode::UnsupportedVersion); }
        envelope.request.validate()?;
        let request = envelope.request.clone();
        let id = envelope.request_id;
        if !matches!(request, Request::Status) && envelope.epoch != Some(self.epoch) { return Err(ErrorCode::StaleSession); }
        let digest = envelope.token.as_deref().map(|token| {
            if token.len() != 64 || !token.bytes().all(|b| b.is_ascii_hexdigit()) { return Err(ErrorCode::Unauthorized); }
            Ok(token_hash(token))
        }).transpose()?;
        let auth = self.transaction(move |broker, state| {
            let auth = digest.and_then(|digest| state.registry.peers.iter().find(|p| equal_digest(&p.token_hash, &digest)).map(|p| Auth { id: p.id, digest }));
            if digest.is_some() && auth.is_none() { return Err(ErrorCode::Unauthorized); }
            if !matches!(request, Request::Status) { broker.ready(state)?; }
            Ok(auth)
        }).await?;
        match envelope.request.clone() {
            Request::Status => {
                let client_id = auth.map(|a| a.id);
                self.transaction(move |broker, state| Ok(Response::Status(ServiceStatus {
                    epoch: broker.epoch, client_id, ready: !state.faulted && !broker.shutdown.is_cancelled(), effects: Vec::new(),
                }))).await
            },
            Request::Pair(params) => {
                if auth.is_some() { return Err(ErrorCode::InvalidRequest); }
            ...
            request => {
                ...
                match request {
                    ...
                    Request::ListCredentials => self.transaction(move |broker, state| {
                        let peer = broker.peer(state, &auth)?;
                        let mut rows = peer.references.iter().filter_map(|r| broker.metadata(r)).collect::<Vec<_>>();
                        rows.sort_by(|a, b| a.label.cmp(&b.label).then(a.credential_ref.cmp(&b.credential_ref)));
                        Ok(Response::Credentials(rows))
                    }).await,
                    Request::RequestAccess(params) => self.request_access(auth, id, params).await,
                    Request::ApprovalStatus(query) => self.transaction(move |broker, state| {
                        broker.peer(state, &auth)?;
                        let job = state.jobs.get(&query.approval_id).filter(|j| j.owner == auth.id).ok_or(ErrorCode::NotFound)?;
                        Ok(Response::Approval(job.projection(query.approval_id)))
                    }).await,
                    Request::RevokeClient(params) => self.revoke(auth, id, params).await,
                    Request::Shutdown => {
                        let _permit = Arc::clone(&self.human_gate).try_acquire_owned().map_err(|_| ErrorCode::Busy)?;
                        let auth2 = auth.clone();
                        let label = self.transaction(move |b, s| Ok(b.peer(s, &auth2)?.label.clone())).await?;
                        ...
                        self.transaction(move |b, s| { b.peer(s, &auth)?; b.audit(s, "standalone_stopping", id)?; b.shutdown.cancel(); Ok(Response::Stopping) }).await
                    },
    ...
    async fn pair(self: &Arc<Self>, id: Uuid, params: PairRequest) -> Result<Response, ErrorCode> {
        ...
        self.transaction(move |broker, state| {
            broker.ready(state)?;
            if state.registry.peers.len() >= MAX_CLIENTS { return Err(ErrorCode::Capacity); }
            if state.registry.peers.iter().any(|p| p.label == proposed) { return Err(ErrorCode::Conflict); }
            Ok(())
        }).await?;
        ...
        self.transaction(move |broker, state| {
            broker.ready(state)?;
            if state.registry.peers.len() >= MAX_CLIENTS || state.registry.peers.iter().any(|p| p.label == label) { return Err(ErrorCode::Conflict); }
            broker.audit(state, "standalone_pair_requested", id)?;
            state.registry.peers.push(Peer { id: client_id, label, token_hash: digest, references: BTreeSet::new() });
            broker.save(state)?;
            broker.audit(state, "standalone_paired", id)?;
            Ok(())
        }).await?;
        Ok(Response::Paired(Pairing { client_id, token: token.to_string() }))
    }

    async fn enroll(self: &Arc<Self>, auth: Auth, id: Uuid, params: EnrollRequest) -> Result<Response, ErrorCode> {
        let _permit = Arc::clone(&self.human_gate).try_acquire_owned().map_err(|_| ErrorCode::Busy)?;
        let reference = format!("cred_{id}");
        let auth2 = auth.clone();
        let ref2 = reference.clone();
        let peer_label = self.transaction(move |b, s| {
            let peer = b.peer(s, &auth2)?;
            if b.store.contains_provisioned(&ref2) { return Err(ErrorCode::Conflict); }
        ...
        self.transaction(move |broker, state| {
            ...
            state.registry.peers.iter_mut().find(|p| p.id == auth.id).ok_or(ErrorCode::Unauthorized)?.references.insert(reference.clone());
            broker.save(state)?;
            broker.audit(state, "standalone_enrolled", id)?;
            Ok(Response::Enrolled(broker.metadata(&reference).ok_or(ErrorCode::Unavailable)?))
        }).await
    }

    async fn request_access(self: &Arc<Self>, auth: Auth, id: Uuid, params: AccessRequest) -> Result<Response, ErrorCode> {
        let auth2 = auth.clone();
        ...
        tokio::spawn(async move {
            ...
            let _ = broker.transaction(move |b, state| {
                if b.ready(state).is_err() {
                    if let Some(job) = state.jobs.get_mut(&id) { job.decision = Decision::Denied; }
                    return Ok(());
                }
                ...
                if decision == Decision::Allowed {
                    if let Some(peer) = state.registry.peers.iter_mut().find(|p| p.id == auth.id) { peer.references.insert(reference); }
                    if b.save(state).is_err() { if let Some(job) = state.jobs.get_mut(&id) { job.decision = Decision::Uncertain; } return Err(ErrorCode::PersistenceUncertain); }
                }
                if b.audit(state, "standalone_metadata_access_decided", id).is_err() {
                    if let Some(job) = state.jobs.get_mut(&id) { job.decision = Decision::Uncertain; }
                    return Err(ErrorCode::PersistenceUncertain);
                }
                if let Some(job) = state.jobs.get_mut(&id) { job.decision = decision; }
                Ok(())
            }).await;
        });
    ...
    async fn revoke(self: &Arc<Self>, auth: Auth, id: Uuid, params: RevokeRequest) -> Result<Response, ErrorCode> {
        let _permit = Arc::clone(&self.human_gate).try_acquire_owned().map_err(|_| ErrorCode::Busy)?;
        let auth2 = auth.clone();
        let client_id = params.client_id;
        let (requester, target) = self.transaction(move |b, s| {
            let requester = b.peer(s, &auth2)?.label.clone();
            let target = s.registry.peers.iter().find(|p| p.id == client_id).ok_or(ErrorCode::NotFound)?.label.clone();
            Ok((requester, target))
        }).await?;
        ...
        self.transaction(move |b, s| {
            ...
            for job in s.jobs.values_mut().filter(|j| j.owner == client_id) { job.decision = Decision::Denied; }
            b.save(s)?;
...
mod tests {
    ...
    fn configured_registry_capacity_fits_the_persistence_bound() {
        let references = (0..MAX_CREDENTIALS).map(|i| format!("cred_{}",Uuid::from_u128(i as u128))).collect::<BTreeSet<_>>();
        let registry = Registry {version:1,peers:(0..MAX_CLIENTS).map(|i| Peer {
            id:Uuid::from_u128(i as u128),label:"x".repeat(80),token_hash:[255;32],references:references.clone(),
        }).collect()};
    ...
    fn consent_expiry_and_retention_use_monotonic_deadlines() {
        let now = Instant::now();
        let id = Uuid::new_v4();
        let mut state = State {registry:Registry {version:1,peers:vec![]},jobs:HashMap::new(),faulted:false};
        state.jobs.insert(id,Job {owner:Uuid::new_v4(),reference:format!("cred_{}",Uuid::new_v4()),decision:Decision::Pending,expires:now,retain_until:now + Duration::from_secs(1)});
        state.reap(now);
