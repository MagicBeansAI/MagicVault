...
impl Client {
    ...
    pub async fn status(&self) -> Result<ServiceStatus, ErrorCode> {
        match self.send(Request::Status, None, Uuid::new_v4()).await? {
            ...
            _ => Err(ErrorCode::TransportUncertain),
        }
    }
    ...
    async fn send(
        ...
        let envelope = Envelope {
            ...
            request_id,
            epoch,
            token: self.pairing.as_ref().map(|p| p.token.clone()),
            request,
        };
    ...
    pub async fn pair(root: PathBuf, profile: &str, label: String) -> Result<Uuid, ErrorCode> {
        ...
        let Response::Paired(pairing) = client.call(Request::Pair(PairRequest { label })).await?
        else {
            return Err(ErrorCode::TransportUncertain);
        };
        let bytes =
            Zeroizing::new(serde_json::to_vec(&pairing).map_err(|_| ErrorCode::Unavailable)?);
...
fn profile_path(root: &Path, profile: &str) -> PathBuf {
    root.join(format!("client-{profile}.json"))
}
