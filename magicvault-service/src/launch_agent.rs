...
pub async fn start(root: &Path) -> Result<(), ErrorCode> {
    ...
    launchctl(&[
        ...
        path.to_str().ok_or(ErrorCode::InvalidRequest)?.into(),
    ])
    .await
}
...
fn verify_definition(
    ...
    root: &Path,
    executable: &Path,
) -> Result<Option<PathBuf>, ErrorCode> {
    ...
    if storage::read_private(path, 16 * 1024)?.as_slice()
        != plist(executable, root, label)?.as_bytes()
    {
        return Err(ErrorCode::Conflict);
    }
...
mod ownership_tests {
    ...
    fn lifecycle_requires_exact_private_definition_not_just_a_marker() {
        ...
        assert!(verify_definition(&file, "fixture", vault, exe)
            .unwrap()
            .is_none());
        ...
        assert!(verify_definition(&file, "fixture", vault, exe)
            ...
            .is_some());
        assert!(
            verify_definition(&file, "fixture", vault, Path::new("/tmp/other-executable")).is_err()
        );
