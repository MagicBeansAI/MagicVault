...
fn initialize_with_key(
    root: &Path,
    initialize_key: impl FnOnce(&Instance) -> Result<(), ErrorCode>,
...
impl MasterKeyProvider for CachedKey {
    ...
    fn delete_key(&self) -> Result<(), SecretEncryptionError> {
        Err(SecretEncryptionError::Keychain(
            "standalone key rotation requires explicit recovery".into(),
        ))
    }
...
fn create_key(instance: &Instance) -> Result<(), ErrorCode> {
    ...
    match entry.get_password() {
        ...
        Ok(value) => {
            let _guard = Zeroizing::new(value);
            return Err(ErrorCode::Conflict);
        }
        Err(_) => return Err(ErrorCode::Unavailable),
    }
...
mod initialization_tests {
    ...
    fn key_failure_does_not_publish_identity_and_retry_recovers_only_the_empty_setup() {
        ...
        assert!(matches!(
            initialize_with_key(&root, |_| Err(ErrorCode::Unavailable)),
            Err(ErrorCode::Unavailable)
        ));
        ...
        let existing = initialize_with_key(&root, |_| {
            panic!("existing identity must never regenerate its key")
        })
        .unwrap();
        assert_eq!(created.id, existing.id);
        assert_eq!(inspect_instance(&root).unwrap().id, created.id);
        assert_eq!(
            fs::metadata(root.join("instance.json"))
                .unwrap()
                .permissions()
                .mode()
                & 0o777,
    ...
    fn unrelated_state_is_never_adopted_or_sent_to_a_key_backend() {
        ...
        assert!(matches!(
            initialize_with_key(root.path(), |_| panic!("unrecognized root")),
            Err(ErrorCode::Conflict)
        ));
        assert_eq!(
            fs::read(root.path().join("retained")).unwrap(),
            b"existing state"
        );
    }
}
